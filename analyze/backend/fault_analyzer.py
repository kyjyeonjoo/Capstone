...
def get_compatible_event_types(event_types: List[str]) -> List[str]:
    """내부 세부 이벤트를 기존 화면/DB에서 사용하는 이벤트 이름으로 변환합니다."""
    compatible = []
...
def detect_events(supabase: Client, video_id: int, records: List[Dict], fps: float = DEFAULT_FPS) -> Tuple[List[str], Dict[str, List[int]]]:
    ...
    lane_side_history: Dict[int, List[Tuple[int, float, bool]]] = {}
        ...
        if len(whitelines) > 0:
            for v in vehicles:
                ...
                lane_side_history.setdefault(v_track_id, []).append(
                    (frame_no, vehicle_cx - line_cx, check_overlap(v, nearest_line))
                )
    ...
    # 차선 중심을 기준으로 차량 중심이 충분한 거리만큼 양쪽을 통과한 경우만 진로변경으로 확정
    for track_id, observations in lane_side_history.items():
        observations.sort(key=lambda item: item[0])
        negative = [item for item in observations if item[1] <= -MIN_LANE_CROSSING_PX]
        positive = [item for item in observations if item[1] >= MIN_LANE_CROSSING_PX]
        has_overlap = any(item[2] for item in observations)
        if not negative or not positive or not has_overlap:
            continue
        crossing_frame = min(
            max(negative[0][0], positive[0][0]),
            observations[-1][0],
        )
    ...
    for track_id, observations in yellow_line_history.items():
        ...
        negative = [item for item in observations if item[1] <= -MIN_LANE_CROSSING_PX]
        positive = [item for item in observations if item[1] >= MIN_LANE_CROSSING_PX]
        has_overlap = any(item[3] for item in observations)
        if len(observations) < 3 or not negative or not positive or not has_overlap:
            continue

        crossing_observation = min(observations, key=lambda item: abs(item[1]))
        crossing_frame, _, crossing_x, _ = crossing_observation
    ...
    max_frame = max((rec.get("frame", 0) for rec in records), default=0)
    max_y2 = max((rec.get("bbox_y2", 0) for rec in records), default=720)
    image_area = max(1, width * max_y2)
    ...
    if max_frame > 0:
        for track_id, boxes in track_boxes.items():
            ...
            starts_near_road = start[4] >= max_y2 * 0.55
            becomes_close = (
                closest[3] >= image_area * ROADSIDE_ENTRY_MIN_FINAL_AREA_RATIO
                and closest[4] >= max_y2 * 0.82
            )
            if not (
    ...
    if roadside_entry_track is not None:
        ...
        for rec in records:
            if rec.get("track_id") == roadside_entry_track:
                rec.setdefault("party", "B")
    ...
    for track_id, boxes in track_boxes.items():
        ...
        approaches_camera = end[4] >= max_y2 * 0.65
        if not (
            appears_late
            and starts_at_side
            and moves_inward
    ...
    if not seen_events and max_frame > 0:
        ...
        for track_id, boxes in track_boxes.items():
            ...
            approaches_camera = closest[4] >= max_y2 * 0.82
            if not (
                persists_to_end
                and approaches_camera
    ...
    has_crosswalk_or_stopline = bool(intersection_markers)
...
def match_accident_type(supabase: Client, event_types: List[str], records: List[Dict]) -> Optional[Dict]:
    ...
    has_signal_object = any(
        get_base_type(rec.get("object_type"))
        in ("red", "green_light", "yellow_light", "left_light")
        for rec in records
    )
    has_stopline = any(
        get_base_type(rec.get("object_type")) in ("stopline", "corner_stopline")
        for rec in records
    )
    has_signal_context = has_signal_object and has_stopline
...
def analyze_fault(
    ...
    return {
        **result,
        "detected_events":    compatible_event_types,
